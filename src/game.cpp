...
bool Game::setup()
{
    ...
    case SetupType::kStandard:
        ...
        bool has_vagabond1 = std::find(factions.begin(), factions.end(), FactionID::kVagabond1) != factions.end();
        bool has_vagabond2 = std::find(factions.begin(), factions.end(), FactionID::kVagabond2) != factions.end();

        while (factions.size() < player_count - 1)
        {
            FactionID potential_faction = static_cast<FactionID>(faction_dist(rng));

            if (potential_faction == FactionID::kVagabond1)
            {
                if (!has_vagabond1)
                {
                    factions.push_back(FactionID::kVagabond1);
                    has_vagabond1 = true;
                }
            }
            else if (potential_faction == FactionID::kVagabond2)
            {
                if (double_vagabond && has_vagabond1 && !has_vagabond2)
                {
                    factions.push_back(FactionID::kVagabond2);
                    has_vagabond2 = true;
                }
            }
            else
        ...
        auto marquise_pointer = std::find(factions.begin(), factions.end(), static_cast<FactionID>(FactionID::kMarquiseDeCat));
        ...
        if (marquise_pointer != factions.end())
        {
            ...
            {
                if (j != kOppositeCorner[starting_clearing_indices.at(marquise_index)])
                {
                    clearings[j].set_pawn_count(FactionID::kMarquiseDeCat, 1);
                }
            }
            ...
            for (uint8_t j = 0; j < 12; ++j)
            {
                if (clearings[starting_clearing_indices.at(marquise_index)].get_clearing_connection(j) == ClearingConnection::kNormal)
                {
                    starting_buildings_indices[j] = clearings[j].get_next_empty_building_slot_index(0);
                }
            }

            setup_marquise_de_cat_starting_building(Building::kSawmill, starting_buildings_indices);
            setup_marquise_de_cat_starting_building(Building::kWorkshop, starting_buildings_indices);
            setup_marquise_de_cat_starting_building(Building::kRecruiter, starting_buildings_indices);
        }

        auto eyrie_pointer = std::find(factions.begin(), factions.end(), static_cast<FactionID>(FactionID::kEyrieDynasties));
        ...
        if (eyrie_pointer != factions.end())
        {
            ...
            {
                starting_clearing_indices.at(eyrie_index) = kOppositeCorner[starting_clearing_indices.at(marquise_index)];
            }

            clearings[starting_clearing_indices.at(eyrie_index)].set_pawn_count(FactionID::kEyrieDynasties, 6);
